...
Architecture (default hidden_units=10, input 3×64×64):
  Block 1 : Conv(3→60,  3×3, p=1) → BN → ReLU
            Conv(60→120, 3×3, s=2, p=1) → BN → ReLU → MaxPool(2) → 120×16×16
  Block 2 : Conv(120→80, 3×3, p=1) → BN → ReLU
            Conv(80→120, 3×3, p=1) → BN → ReLU → MaxPool(2) → 120×8×8
  Block 3 : Conv(120→80, 3×3, p=1) → BN → ReLU
            Conv(80→10,  3×3, p=1) → BN → ReLU → MaxPool(2) → 10×4×4
...
class FurnitureClassifier(nn.Module):
    def __init__(self, in_channels: int = 3, hidden_units: int = 10, num_classes: int = 4):
        ...
        self.block_1 = nn.Sequential(
            ...
            nn.BatchNorm2d(hidden_units * 12),
            nn.ReLU(),
